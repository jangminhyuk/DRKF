...
def get_color_for_filter(filt, i):
    """Define consistent color mapping for all plots"""
...
def create_theta_effect_plot(all_results, dist, filters, filter_labels):
    ...
    # Customize plot
    ax.set_xlabel('θ')
    ax.set_ylabel('Average MSE', fontsize=20)
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.grid(True, alpha=0.3)
...
def create_regret_theta_effect_plot(all_results, dist, filters, filter_labels):
    ...
    # Customize plot
    ax.set_xlabel('θ')
    ax.set_ylabel('Average Regret MSE', fontsize=20, labelpad=15)
    ax.set_xscale('log')
    ax.grid(True, alpha=0.3)
...
def main(dist):
    ...
    filter_labels = {
        ...
        'drkf_inf_cdc': "Steady-State DRKF (CDC2025)",
