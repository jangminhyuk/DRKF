...
def compute_envelope_scalars(Sigma_hat, theta, nominal_matrix=None):
    ...
    -----------
    Sigma_hat : ndarray
        Current time-step covariance matrix (not used when nominal_matrix provided)
    theta : float
        Wasserstein radius (non-negative)
    nominal_matrix : ndarray, optional
        Nominal matrix to use for both lower and upper bound calculations
        (should be nominal_Sigma_w for process noise, nominal_Sigma_v for measurement noise)
        
    Returns:
    --------
    underline_lambda : float
        Lower envelope scalar: λ_min(nominal_matrix)
    overline_lambda : float  
        Upper envelope scalar: (√λ_max(nominal_matrix) + θ)²
    """
    if nominal_matrix is not None:
        # Use the nominal matrix for both bounds
        # Symmetrize to ensure real eigenvalues
        nominal_matrix = (nominal_matrix + nominal_matrix.T) / 2
        
        # Compute eigenvalues of nominal matrix
        ...
        # Lower bound: λ_min(nominal_matrix) - correct envelope lower bound
        underline_lambda = lambda_min_nominal
        
        # Upper bound: (√λ_max(nominal_matrix) + θ)² - correct envelope upper bound
