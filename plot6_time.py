...
This script creates system dimension scalability plots from the results of main6_time.py.
Shows execution time vs system dimension for drkf_inf method.

Usage:
    python plot6_time.py
"""
...
# Set larger font sizes for better readability (matching plot_time.py)
plt.rcParams.update({
...
def create_scalability_plot():
    ...
    # Customize plot
    ax.set_xlabel('System Dimension', fontsize=26)
    ax.set_ylabel('Computation time (s)', fontsize=26)
    ...
    # Add more ticks for y-axis with decimal formatting
    from matplotlib.ticker import LogLocator, FuncFormatter
    ax.yaxis.set_major_locator(LogLocator(base=10, numticks=10))
    ax.yaxis.set_minor_locator(LogLocator(base=10, subs=np.arange(2, 10), numticks=20))
    
    # Custom formatter to show decimal notation instead of scientific notation
    def format_func(x, p):
        if x >= 1:
