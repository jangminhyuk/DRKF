...
Usage example:
    ...
    python main2.py --dist quadratic
    python main2.py --dist laplace
"""
...
from common_utils import (save_data, is_stabilizable, is_detectable, is_positive_definite,
                         enforce_positive_definiteness, generate_desired_trajectory, compute_lqr_cost)
...
def run_experiment(exp_idx, dist, num_sim, seed_base, robust_val, T_total, desired_traj):
    ...
    mse_mean_drkf_finite = np.mean([np.mean(r[0]['mse']) for r in results_drkf_finite])
    cost_mean_drkf_finite = np.mean([r[1] for r in results_drkf_finite])
    rep_state_drkf_finite = results_drkf_finite[0][0]['state_traj']

    results_drkf_inf_cdc = [run_simulation_inf_drkf_cdc(i) for i in range(num_sim)]
    mse_mean_drkf_inf_cdc = np.mean([np.mean(r[0]['mse']) for r in results_drkf_inf_cdc])
    cost_mean_drkf_inf_cdc = np.mean([r[1] for r in results_drkf_inf_cdc])
    rep_state_drkf_inf_cdc = results_drkf_inf_cdc[0][0]['state_traj']

    results_drkf_finite_cdc = [run_simulation_finite_drkf_cdc(i) for i in range(num_sim)]
    mse_mean_drkf_finite_cdc = np.mean([np.mean(r[0]['mse']) for r in results_drkf_finite_cdc])
    cost_mean_drkf_finite_cdc = np.mean([r[1] for r in results_drkf_finite_cdc])
    rep_state_drkf_finite_cdc = results_drkf_finite_cdc[0][0]['state_traj']

    results_drkf_neurips = [run_simulation_drkf_neurips(i) for i in range(num_sim)]
    mse_mean_drkf_neurips = np.mean([np.mean(r[0]['mse']) for r in results_drkf_neurips])
    cost_mean_drkf_neurips = np.mean([r[1] for r in results_drkf_neurips])
    rep_state_drkf_neurips = results_drkf_neurips[0][0]['state_traj']
    ...
    overall_results = {
        ...
        'cost': {
            ...
            'drkf_finite': cost_mean_drkf_finite,
            'drkf_inf_cdc': cost_mean_drkf_inf_cdc,
            'drkf_finite_cdc': cost_mean_drkf_finite_cdc,
            'drkf_neurips': cost_mean_drkf_neurips,
            'bcot': cost_mean_bcot,
            'risk': cost_mean_risk
        }
    }
...
def main(dist, num_sim, num_exp, T_total):
    ...
    filters = ['finite', 'inf', 'drkf_inf', 'drkf_finite', 'drkf_inf_cdc', 'drkf_finite_cdc', 'drkf_neurips', 'bcot', 'risk']
    filter_labels = {
        ...
        'drkf_neurips': "DR Kalman filter (NeurIPS version)"
    }
    ...
    for f in filters:
        if f in ['finite', 'inf']:
            ...
            optimal_results[f] = {
                'robust_val': best_val,
                'cost': all_results[best_val]['cost'][f],
    ...
    for filt, info in sorted_optimal:
        print(f"{filt}: Optimal robust parameter = {info['robust_val']}, LQR cost = {info['cost']:.4f} ({info['cost_std']:.4f}), Average MSE = {info['mse']:.4f} ({info['mse_std']:.4f})")
    
    results_path = "./results/estimator/"
    if not os.path.exists(results_path):
        os.makedirs(results_path)
    save_data(os.path.join(results_path, f'overall_results_{dist}_{dist}_cdc.pkl'), all_results)
    save_data(os.path.join(results_path, f'optimal_results_{dist}_{dist}_cdc.pkl'), optimal_results)
    # Save raw experiments data.
    save_data(os.path.join(results_path, f'raw_experiments_{dist}_{dist}_cdc.pkl'), raw_experiments_data)
    print("LQR with state estimation experiments completed for all robust parameters.")
    
    # --- Print Readable Data for the User ---
    print("\nDetailed Experiment Results:")
    header = "{:<50} {:<35} {:<35} {:<15}".format("Method", "LQR Cost", "Average MSE", "Best theta")
    print(header)
    print("-"*135)
    for filt in filters:
        best_theta = optimal_results[filt]['robust_val']
        cost = optimal_results[filt]['cost']
        cost_std = optimal_results[filt]['cost_std']
        mse = optimal_results[filt]['mse']
        mse_std = optimal_results[filt]['mse_std']
        cost_str = f"{cost:.1f} ({cost_std:.1f})"
        mse_str = f"{mse:.3f} ({mse_std:.3f})"
        print("{:<50} {:<35} {:<35} {:<15}".format(filter_labels[filt], cost_str, mse_str, best_theta))
